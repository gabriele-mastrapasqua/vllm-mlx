...
class TestClearCacheInterval:
    ...
    def test_clear_cache_interval_configured(self):
        ...
        assert scheduler._clear_cache_interval == 16
...
class TestIncrementalCacheEval:
    ...
    @patch("vllm_mlx.scheduler.mx")
    def test_eager_eval_in_extraction_path(self, mock_mx):
        """Verify mx.eval is called during cache extraction to force evaluation."""
        scheduler = _make_scheduler()
        ...
        # Verify mx.eval was called (forces evaluation of cached tensors
        # to prevent deferred VRAM spike during later access)
        assert mock_mx.eval.called, "mx.eval should be called during cache extraction"
