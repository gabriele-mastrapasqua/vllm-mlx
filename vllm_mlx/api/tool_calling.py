...
def parse_tool_calls(text: str) -> Tuple[str, Optional[List[ToolCall]]]:
    ...
    # Pattern for bare function with parameter tags (Qwen3 style):
    # <function=name><parameter=key>value</parameter>...</function>
    if not tool_calls:
        bare_func_pattern = r"<function=([^>]+)>(.*?)</function>\s*(?:</tool_call>)?"
        bare_func_matches = re.findall(bare_func_pattern, cleaned_text, re.DOTALL)
        param_pattern = r"<parameter=([^>]+)>\s*(.*?)\s*</parameter>"
        for name, params_block in bare_func_matches:
            params = re.findall(param_pattern, params_block, re.DOTALL)
            if params:  # Only match if there are <parameter> tags
                arguments = {}
                for p_name, p_value in params:
                    val = p_value.strip()
                    try:
